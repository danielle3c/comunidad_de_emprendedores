...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $data = [
        ...
        'fecha_termino'  => filter_input(INPUT_POST, 'fecha_termino', FILTER_SANITIZE_STRING) ?: null,
        'monto_total'    => (float)str_replace(',', '.', filter_input(INPUT_POST, 'monto_total', FILTER_SANITIZE_STRING) ?? 0),
        'descripcion'    => sanitize(filter_input(INPUT_POST, 'descripcion', FILTER_SANITIZE_STRING) ?? ''),
        'tipo_contrato'  => sanitize(filter_input(INPUT_POST, 'tipo_contrato', FILTER_SANITIZE_STRING) ?? ''),
    ...
    try {
        if ($postId) {
            ...
            $sql = "INSERT INTO Contratos 
                    (emprendedores_idemprendedores, fecha_inicio, fecha_termino, monto_total, descripcion, tipo_contrato, estado)
                    VALUES 
                    (:emprendedores_idemprendedores, :fecha_inicio, :fecha_termino, :monto_total, :descripcion, :tipo_contrato, :estado)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($data);
        ...
        error_log("Error en contratos.php: " . $e->getMessage());
    }
    redirect('contratos.php');
}
...
if ($action === 'delete' && $id) {
    try {
        $pdo->prepare("DELETE FROM Contratos WHERE idContratos = ?")->execute([$id]);
        setFlash('success', 'Contrato eliminado.');
    } catch (PDOException $e) {
        setFlash('error', 'No se puede eliminar: ' . $e->getMessage());
        error_log("Error en contratos.php (delete): " . $e->getMessage());
    }
    redirect('contratos.php');
}
...
$edit = null;
if ($action === 'edit' && $id) {
    $stmt = $pdo->prepare("SELECT * FROM Contratos WHERE idContratos = ?");
    $stmt->execute([$id]);
    $edit = $stmt->fetch();
    if (!$edit) {
        setFlash('error', 'Contrato no encontrado.');
        redirect('contratos.php');
    }
}
...
if ($search) {
    $conditions[] = "(p.nombres LIKE :search OR p.apellidos LIKE :search OR c.tipo_contrato LIKE :search)";
...
$sql = "SELECT c.*, CONCAT(p.nombres,' ',p.apellidos) AS nombre_persona $base ORDER BY c.fecha_inicio DESC LIMIT :limit OFFSET :offset";
$stmt = $pdo->prepare($sql);
...
<div class="card mb-4">
    ...
    <form method="POST">
        ...
        <input type="hidden" name="id" value="<?= htmlspecialchars($edit['idContratos'] ?? '') ?>">
        <div class="row g-3">
            <div class="col-md-4">
            ...
            <div class="col-md-2">
                ...
                <input type="date" name="fecha_termino" class="form-control form-control-sm" 
                       value="<?= htmlspecialchars($edit['fecha_termino'] ?? '') ?>">
            </div>
            <div class="col-md-2">
                <label class="form-label">Monto Total *</label>
                <input type="number" step="0.01" name="monto_total" class="form-control form-control-sm" 
                       value="<?= htmlspecialchars($edit['monto_total'] ?? '') ?>" required>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-3">
                ...
                <input type="text" name="tipo_contrato" class="form-control form-control-sm" 
                       value="<?= htmlspecialchars($edit['tipo_contrato'] ?? '') ?>">
            </div>
            <div class="col-md-9">
...
<div class="card">
    ...
    <table class="table table-hover mb-0">
        ...
        <tr>
            <td><?= (int)$r['idContratos'] ?></td>
            <td><?= htmlspecialchars($r['nombre_persona']) ?></td>
            <td><?= htmlspecialchars($r['tipo_contrato']) ?: '-' ?></td>
            <td><?= formatDate($r['fecha_inicio']) ?></td>
            <td><?= formatDate($r['fecha_termino']) ?></td>
            <td><?= formatMoney($r['monto_total']) ?></td>
            <td><?= badgeEstado($r['estado']) ?></td>
            <td>
                <a href="contratos.php?action=edit&id=<?= (int)$r['idContratos'] ?>" class="btn btn-sm btn-outline-primary btn-action"><i class="bi bi-pencil"></i></a>
                <a href="contratos.php?action=delete&id=<?= (int)$r['idContratos'] ?>" class="btn btn-sm btn-outline-danger btn-action btn-delete"><i class="bi bi-trash"></i></a>
            </td>
        </tr>
        <?php endforeach; ?>
